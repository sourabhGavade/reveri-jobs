...
trait CompanyPackageTrait
{
    ...
    public function addCompanySearchPackage($company, $package)
    {
        ...
        $company->cvs_quota = $package->num_data_access;
    ...
    public function updateCompanySearchPackage($company, $package)
    {
        ...
        $company->cvs_quota = ($company->cvs_quota - $company->availed_cvs_quota) + $package->num_data_access;
        $company->availed_cvs_quota = 0;
        $company->update();
    }
    public function addBothCompanyPackage($company, $package){
        $now = Carbon::now();
        $company->package_id = $package->id;
        $company->package_start_date = $now;
        $company->package_end_date = $now->addDays($package->package_num_days);
    ...
    public function updateBothCompanyPackage($company, $package){
        $package_end_date = $company->package_end_date;
        $current_end_date = Carbon::createFromDate($package_end_date->format('Y'), $package_end_date->format('m'), $package_end_date->format('d'));
        $company->package_id = $package->id;
        $company->package_end_date = $current_end_date->addDays($package->package_num_days);
