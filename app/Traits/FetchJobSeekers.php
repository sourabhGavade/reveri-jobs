...
trait FetchJobSeekers
{
    ...
    public function fetchJobSeekers($search = '', $industry_ids = array(), $functional_area_ids = array(), $country_ids = array(), $state_ids = array(), $city_ids = array(), $career_level_ids = array(), $gender_ids = array(), $job_experience_ids = array(), $current_salary = 0, $expected_salary = 0, $salary_currency = '', $order_by = 'id', $limit = 10)
    {
        ...
        $query=$query->where('users.functional_area_id','!=',null)->orderBy('users.id', 'DESC');
    ...
    {
        $query->where('users.is_active', 1)->where('mobile_num','!=','');
        if ($search != '') {
            $query = $query->whereRaw("MATCH (`search`) AGAINST ('$search*' IN BOOLEAN MODE)");
        }
        if (isset($industry_ids[0])) {
            $query = $query->whereIn('users.industry_id', $industry_ids);
        }
        if (isset($functional_area_ids[0])) {
            $query = $query->whereIn('users.functional_area_id', $functional_area_ids);
        }
        if (isset($country_ids[0])) {
            $query = $query->whereIn('users.country_id', $country_ids);
        }
        if (isset($state_ids[0])) {
            $query = $query->whereIn('users.state_id', $state_ids);
        }
        if (isset($city_ids[0])) {
            $query = $query->whereIn('users.city_id', $city_ids);
        }
        if (isset($career_level_ids[0])) {
            $query = $query->whereIn('users.career_level_id', $career_level_ids);
        }
        if (isset($gender_ids[0])) {
            $query = $query->whereIn('users.gender_id', $gender_ids);
        }
        if (isset($job_experience_ids[0])) {
            $query = $query->whereIn('users.job_experience_id', $job_experience_ids);
        }
        if ((int) $current_salary > 0) {
            $query = $query->where('users.current_salary', '>=', $current_salary);
        }
