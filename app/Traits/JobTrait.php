...
// dd($functionalAreas);
...

    {
        ...
        $job->is_featured = $request->input('is_featured');
         $job->seo_title=$request->seo_title;
        $job->seo_description=$request->seo_description;
        $job->seo_tags=$request->seo_tags;
...

    {
        ...
        $job = $this->assignJobValues($job, $request);
        if($company->package_id==5)
        $job->package_type='classified';
        if($company->package_id==6)
        $job->package_type='advanced';
        $job->save();
        /*         * ******************************* */
