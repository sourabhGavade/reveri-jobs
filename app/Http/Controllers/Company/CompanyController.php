...
class CompanyController extends Controller
{
    ...
    {
        $this->middleware('checkCompanyProfileCompleted',['except'=>['companyProfile','updateCompanyProfile']]);
    ...
    public function updateCompanyProfile(CompanyFrontFormRequest $request)
    {
		...
		$company->is_profile_completed = 1;
    ...
    public function companyDetail(Request $request, $company_slug)
    {
        ...
        /*         * ************************************************** */
         if($company->seo_title != "" && $company->seo_title != null){
            $seo = (object) array(
                'seo_title' => $company->seo_title,
                'seo_description' => $company->seo_description,
                'seo_keywords' => $company->seo_tags,
                'seo_other' => ''
            );
        } else {
    ...
    public function listsuggestedcandidates(Request $request, $job_id)
    {
        // $requested_users = Suggested_request::where('job_id', '=', $job_id)->pluck('users_ids');
        // if ($requested_users->isNotEmpty()) {
        //     $requested_users_ids = json_decode($requested_users, true);
        //     //$requested_users_ids = array_map('intval', explode(',', $requested_users_ids));
        //     dd($requested_users_ids);
        //     $job_applications = JobApply::where('job_id', '=', $job_id)->whereNotIn('user_id',$requested_users)->get();
        //     //dd($job_applications);
        ...
        $job_applications = JobApply::where('job_id', '=', $job_id)->get();
         // dd($job_applications);
        return view('job.suggested_jobapplications',compact('job_applications','job_id'));
                        
    }
    public function savesuggestedcandidates(Request $request)
    {
        $company = Auth::guard('company')->user();
        $users_ids = implode(',', $request->users_ids);
        $Suggested_request = new Suggested_request();
        $Suggested_request->company_id = $company->id;
        $Suggested_request->job_id = $request->job_id;
        $Suggested_request->users_ids = $users_ids;
        $Suggested_request->request_send = 1;
        $Suggested_request->save();
    ...
    public function companyTransaction(){
        // die('hello');
        $data=RazorpayOrder::select('rozorpay_orders.*','packages.package_title')
        ->where('rozorpay_orders.company_id',Auth::guard('company')->user()->id)
        ->join('packages','rozorpay_orders.package_id','packages.id')->get();
        // dd($data);die;
        return view('company.transaction_details')
        ->with('data',$data);
    }
