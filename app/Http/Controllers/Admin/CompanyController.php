...
class CompanyController extends Controller
{
    ...
    public function storeCompany(CompanyFormRequest $request)
    {
        ...
        $company->is_approved = $request->input('is_approved');
         $company->seo_description = $request->input('seo_description');
        $company->seo_tags = $request->input('seo_tags');
        $company->seo_title = $request->input('seo_title');
        $company->save();
        /*         * ******************************* */
    ...
    public function fetchCompaniesData(Request $request)
    {
        ...
        return Datatables::of($companies)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('is_approved') && $request->is_approved != -1) {
                                $query->where('companies.is_approved', '=', "{$request->get('is_approved')}");
                            }
                        })
                        ...
                        ->addColumn('is_approved', function ($companies) {
                            return ((bool) $companies->is_approved) ? 'Yes' : 'No';
                        })
                        ->addColumn('action', function ($companies) {
                            ...
                            $approvedTxt = 'Make Approved';
                            $approvedHref = 'makeApproved(' . $companies->id . ');';
                            $approvedIcon = 'square-o';
                            if ((int) $companies->is_approved == 1) {
                                $approvedTxt = 'Make Not Approved';
                                $approvedHref = 'makeNotApproved(' . $companies->id . ');';
                                $approvedIcon = 'check-square-o';
                            }
                            return '
				<div class="btn-group">
...
<li><a href="javascript:void(0);" onClick="' . $approvedHref . '" id="onclickapproved' . $companies->id . '"><i class="fa fa-' . $approvedIcon . '" aria-hidden="true"></i>' . $approvedTxt . '</a></li>
					</ul>
				</div>';
                        })
                        ->rawColumns(['action', 'is_active', 'is_featured', 'is_approved'])
    ...
    public function makeApprovedCompany(Request $request)
    {
        $id = $request->input('id');
        try {
            $company = Company::findOrFail($id);
    ...
    public function makeNotApprovedCompany(Request $request)
    {
        $id = $request->input('id');
        try {
            $company = Company::findOrFail($id);
