...
class AdminController extends Controller
{
    ...
    public function indexAdminSubscribers()
    {
        return view('admin.admin.subscribers');
    }
    ...
    function fetchAdminSubscribersData()
    {
        ...
        return Datatables::of($subscribers)
                        ...
                        ->editColumn('created_at', function($col) {
                        	return [
                        		'display' =>($col->created_at && $col->created_at != '0000-00-00 00:00:00') ? with(new Carbon($col->created_at))->format('d-m-Y') : '',
                        		'timestamp' =>($col->created_at && $col->created_at != '0000-00-00 00:00:00') ? with(new Carbon($col->created_at))->timestamp : ''
                        	];
                        })
                        ->setRowId(function($subscriber) {
                            return 'admin_user_dt_row_' . $subscriber->id;
                        })
                        ->make(true);
    }
