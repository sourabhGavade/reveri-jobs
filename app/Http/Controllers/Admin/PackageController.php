...
class PackageController extends Controller
{
    ...
    public function storePackage(PackageFormRequest $request)
    {
        ...
        $package->num_data_access = $request->input('package_num_listings');
        $package->package_for = $request->input('package_for');
        $package->num_job_position = $request->input('num_job_position');
        $package->num_functional_area = $request->input('num_functional_area');
        $package->num_industry = $request->input('num_industry');
        $package->save();
        /*         * ************************************ */
    ...
    public function updatePackage($id, PackageFormRequest $request)
    {
        ...
        $package->package_num_days = $request->input('package_num_days');
        $package->package_num_listings = $request->input('package_num_listings');
        $package->num_data_access = $request->input('num_data_access');
        $package->package_for = $request->input('package_for');
        $package->num_job_position = $request->input('num_job_position');
        $package->num_functional_area = $request->input('num_functional_area');
        $package->num_industry = $request->input('num_industry');
    ...
    public function fetchPackagesData(Request $request)
    {
        ...
        return Datatables::of($packages)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('num_data_access') && !empty($request->num_data_access)) {
                                $query->where('packages.num_data_access', 'like', "{$request->get('num_data_access')}%");
                            }
