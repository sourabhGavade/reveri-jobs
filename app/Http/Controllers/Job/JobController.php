...
class JobController extends Controller
{
    ...
    {
        // die('hello');
        // $v=Auth::user()->industry_id;
        // dd($v);die;
        $search = $request->query('search', '');
        $job_titles = $request->query('job_title', array());
        $company_ids = $request->query('company_id', array());
        if(Auth::user()){
            $industry_ids = $request->query('industry_id', explode(',',Auth::user()->industry_id));
            $functional_area_ids = $request->query('functional_area_id', explode(',',Auth::user()->functional_area_id));
        }
        else{
        ...
        /*         * ************************************************** */
        if(Auth::user()){
            $industryIdsArray = explode(',',Auth::user()->industry_id);
            $functionalAreaIdsArray = explode(',',Auth::user()->functional_area_id);
    ...
    public function jobDetail(Request $request, $job_slug)
    {
        ...
        if($job->seo_title != '' && $job->seo_title != null){
             $seo = (object) array(
                         'seo_title' => $job->seo_title,
                         'seo_description' => $job->seo_description,
                         'seo_keywords' => $job->seo_tags,
                         'seo_other' => ''
             );
        } else {
