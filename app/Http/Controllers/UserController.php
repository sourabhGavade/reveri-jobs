...
class UserController extends Controller
{
    ...
    function mobileAdd(Request $request)
    {
        $user = User::findOrFail(Auth::user()->id);
        $user->mobile_num = $request->input('mobileNumber');
        $user->update();
    }

    public function updateMyProfile(UserFrontFormRequest $request)
    {
        // dd($request->industry);die;
        // die('hello');
        if($request->isMobAdded == 0)
        {
            $user = User::findOrFail(Auth::user()->id);
            $summary = ProfileSummary::where('user_id',Auth::user()->id)->first()??new ProfileSummary();
            /*         * **************************************** */
            ...
            $user->industry_id = implode(',',$request->industry);
            $user->functional_area_id = implode(',',$request->functional_area);
            ...
            flash(__('Please verify mobile number to continue.'))->error();
            return \Redirect::route('my.profile');
        }
    }
