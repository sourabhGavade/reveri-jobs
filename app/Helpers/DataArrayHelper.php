...
class DataArrayHelper
{
    ...
    {
        $array = Country::select('countries.country', 'countries.country_id')->where('country_id',101)->lang()->active()->sorted()->pluck('countries.country', 'countries.country_id')->toArray();
    ...
    {
        $company = Auth::guard('company')->user(); // (functional_area list display by its package_id) added by Manjunath
    ...
    /*     * **************************** */ 
    public static function defaultPackagesArray() //packages added by manjunath
    {
        $array = Package::select('packages.package_title', 'packages.id')->pluck('packages.package_title', 'packages.id')->toArray();
        return $array;
    }

    public static function langPackagesArray()
    {
        $array = Package::select('packages.package_title', 'packages.id')->pluck('packages.package_title', 'packages.id')->toArray();
        if ((int) count($array) === 0) {
            $array = self::defaultPackagesArray();
        }
        return $array;
    }
