...
<style>
    .page-navigation nav div:nth-child(2){
        display: none;
    }
</style>
<!-- Header end --> 
            ...
            <div class="row"> @include('includes.job_seeker_list_side_bar')                
                <div class="col-lg-6"> 
                    ...
                    <ul class="searchList">
                        ...
                        {{-- @if($jobSeeker->functional_area_id!=null) --}}
                        <li>
                            <div class="row">
                                <div class="col-lg-8 col-md-8">
                                    <div class="jobimg">{{$jobSeeker->printUserImage(100, 100)}}</div>
                                    <div class="jobinfo">
                                        <h3><a href="{{route('user.profile', $jobSeeker->id)}}" @if($view==false)style="pointer-events: none;" @endif>{{$jobSeeker->getName()}}</a></h3>
                                        <div class="location"> {{$jobSeeker->getLocation()}}</div>
                                    </div>
                                    <div class="clearfix">
                                        <p class='mt-1'>
                                            {{\App\CareerLevel::where('id',$jobSeeker->career_level_id)->first()->career_level}}
                                        </p>
                                        @php $industry_id=explode(',',$jobSeeker->industry_id); @endphp
                                        <p class='mt-1'>
                                            @foreach($industry_id as $val)
                                                {{\App\Industry::where('id',$val)->first()->industry}}
                                            @endforeach
                                        </p>
                                        @php $functional_area_id=explode(',',$jobSeeker->functional_area_id); @endphp
                                        <p class='mt-1'>
                                            @foreach($functional_area_id as $val)

                                            {{\App\FunctionalArea::where('id',$val)->first()?\App\FunctionalArea::where('id',$val)->first()->functional_area :(\App\FunctionalArea::where('functional_area_id',$val)->first()?\App\FunctionalArea::where('functional_area_id',$val)->first()->functional_area:'')}}
                                            @endforeach
                                        </p>
                                ...
                                <div class="col-lg-4 col-md-4">
                                    <div class="listbtn"><a href="{{route('user.profile', $jobSeeker->id)}}" @if($view==false)style="pointer-events: none;" @endif>{{__('View Profile')}}</a></div>
                                </div>
                            </div>
                    ...
                    <div class="pagiWrap">
                        <div class="row">
                            ...
                            <div class="col-md-7 text-right page-navigation">
                                @if(isset($jobSeekers) && count($jobSeekers))
