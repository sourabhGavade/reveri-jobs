<div class="col-md-3 col-sm-6"> 
    ...
    <div class="sidebar">
        ...
        <!-- Jobs By Title -->
        <div class="widget">
            <h4 class="widget-title">{{__('Jobs By Position')}}</h4>
            <ul class="optionlist view_more_ul">
                ...
                @php
                $career_level = App\CareerLevel::where('id','=',$career_level_id)->lang()->active()->first();
                @endphp
                @if(null !== $career_level)
                @php
                $checked = (in_array($career_level->id, Request::get('career_level_id', array())))? 'checked="checked"':'';
                @endphp
                <li>
                    <input type="checkbox" name="career_level_id[]" id="career_level_{{$career_level->id}}" value="{{$career_level->id}}" {{$checked}}>
                    <label for="career_level_{{$career_level->id}}"></label>
                    {{$career_level->career_level}} <span>{{App\Job::countNumJobs('career_level_id', $career_level->id)}}</span> </li>
                @endif
                @endforeach
                @endif
            </ul>
            <!-- title end --> 
            <span class="text view_more hide_vm">{{__('View More')}}</span> </div>
        ...
        <div class="widget">
            ...
            <ul class="optionlist view_more_ul">
                ...
                {
                    if(in_array($industry_id, $_GET['industry_id']))
                    {
        ...
        <div class="widget">
            ...
            <ul class="optionlist view_more_ul">
                ...
                @php
                $checked = '';
                if(isset($_GET['functional_area_id']))
                {
                    if(in_array($functional_area_id, $_GET['functional_area_id']))
                    {
                        $checked = "checked=checked";
                    }
                }
